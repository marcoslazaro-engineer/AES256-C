// Code by Marcos Lazaro
#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include <stdlib.h>

#define AES_BLOCK_SIZE 16
#define AES_256_KEY_SIZE 32
#define AES_256_ROUNDS 14
#define Nb 4
#define Nk 8
#define Nr 14

typedef uint8_t state_t[4][4];

// S-box and inverse
static const uint8_t sbox[256] = {
    
    
};

static const uint8_t rsbox[256] = {
    
};

// Rcon table
static const uint8_t Rcon[15] = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36,
    0x6C, 0xD8, 0xAB, 0x4D, 0x9A
};

uint8_t xtime(uint8_t x) {
    return (x << 1) ^ (((x >> 7) & 1) * 0x1b);
}

uint8_t mul(uint8_t a, uint8_t b) {
    uint8_t result = 0;
    for (int i = 0; i < 8; ++i) {
        if (b & 1)
            result ^= a;
        uint8_t hi_bit_set = a & 0x80;
        a <<= 1;
        if (hi_bit_set)
            a ^= 0x1b;
        b >>= 1;
    }
    return result;
}

void InvSubBytes(state_t *state) {
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            (*state)[i][j] = rsbox[(*state)[i][j]];
        }
    }
}

void InvShiftRows(state_t *state) {
    uint8_t temp;
    //inverse second line
    temp = (*state)[1][3];
    (*state)[1][3] = (*state)[1][2];
    (*state)[1][2] = (*state)[1][1];
    (*state)[1][1] = (*state)[1][0];
    (*state)[1][0] = temp;

    // Inverse third line
    temp = (*state)[2][0];
    (*state)[2][0] = (*state)[2][2];
    (*state)[2][2] = temp;
    temp = (*state)[2][1];
    (*state)[2][1] = (*state)[2][3];
    (*state)[2][3] = temp;

    // Inverse fourth line
    temp = (*state)[3][0];
    (*state)[3][0] = (*state)[3][1];
    (*state)[3][1] = (*state)[3][2];
    (*state)[3][2] = (*state)[3][3];
    (*state)[3][3] = temp;
}

void InvMixColumns(state_t *state) {
    for (int c = 0; c < 4; ++c) {
        uint8_t a0 = (*state)[0][c];
        uint8_t a1 = (*state)[1][c];
        uint8_t a2 = (*state)[2][c];
        uint8_t a3 = (*state)[3][c];

        (*state)[0][c] = mul(a0, 0x0e) ^ mul(a1, 0x0b) ^ mul(a2, 0x0d) ^ mul(a3, 0x09);
        (*state)[1][c] = mul(a0, 0x09) ^ mul(a1, 0x0e) ^ mul(a2, 0x0b) ^ mul(a3, 0x0d);
        (*state)[2][c] = mul(a0, 0x0d) ^ mul(a1, 0x09) ^ mul(a2, 0x0e) ^ mul(a3, 0x0b);
        (*state)[3][c] = mul(a0, 0x0b) ^ mul(a1, 0x0d) ^ mul(a2, 0x09) ^ mul(a3, 0x0e);
    }
}

void AddRoundKey(uint8_t round, state_t *state, const uint8_t *RoundKey) {
    for (int col = 0; col < 4; ++col) {
        for (int row = 0; row < 4; ++row) {
            (*state)[row][col] ^= RoundKey[round * Nb * 4 + col * Nb + row];
        }
    }
}

void KeyExpansion(const uint8_t *Key, uint8_t *RoundKey) {
    uint8_t temp[4];
    int i = 0;
    
    for (i = 0; i < Nk * 4; i++) {
        RoundKey[i] = Key[i];
    }
    
    i = Nk;
    
    while (i < Nb * (Nr + 1)) {
        for (int k = 0; k < 4; k++) {
            temp[k] = RoundKey[(i - 1) * 4 + k];
        }
        
        if (i % Nk == 0) {
            uint8_t tmp = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = tmp;
            
            for (int k = 0; k < 4; k++) {
                temp[k] = sbox[temp[k]];
            }
            
            temp[0] ^= Rcon[i / Nk - 1];
        } else if (Nk > 6 && i % Nk == 4) {
            for (int k = 0; k < 4; k++) {
                temp[k] = sbox[temp[k]];
            }
        }
        
        for (int k = 0; k < 4; k++) {
            RoundKey[i * 4 + k] = RoundKey[(i - Nk) * 4 + k] ^ temp[k];
        }
        
        i++;
    }
}

void AES_256_decrypt(const uint8_t *input, const uint8_t *key, uint8_t *output) {
    state_t state;
    uint8_t RoundKey[AES_BLOCK_SIZE * (Nr + 1)];
    KeyExpansion(key, RoundKey);

    
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            state[j][i] = input[i * 4 + j];
        }
    }

    AddRoundKey(Nr, &state, RoundKey);

    for (int round = Nr - 1; round > 0; --round) {
        InvShiftRows(&state);
        InvSubBytes(&state);
        AddRoundKey(round, &state, RoundKey);
        InvMixColumns(&state);
    }

    InvShiftRows(&state);
    InvSubBytes(&state);
    AddRoundKey(0, &state, RoundKey);

    
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            output[i * 4 + j] = state[j][i];
        }
    }
}

void print_hex(const uint8_t *data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        printf("%02x", data[i]);
        if ((i + 1) % 4 == 0) printf(" ");
    }
    printf("\n");
}

void get_user_input(uint8_t *input, uint8_t *key) {
    char hexmsg[2 * AES_BLOCK_SIZE + 2]; // 32 hex + \n + \0
    char password[AES_256_KEY_SIZE + 2]; // 32 + \n + \0

    printf("Put the encrypted message (32 hex digits, no spaces): ");
    if (fgets(hexmsg, sizeof(hexmsg), stdin) == NULL) {
        printf("Error \n");
        exit(1);
    }
    hexmsg[strcspn(hexmsg, "\n")] = 0; // delete line jump

    // simple validation
    if (strlen(hexmsg) != 2 * AES_BLOCK_SIZE) {
        printf("The encrypted message need to have 32 hex digits.\n");
        exit(1);
    }

    for (int i = 0; i < AES_BLOCK_SIZE; ++i) {
        sscanf(hexmsg + 2 * i, "%2hhx", &input[i]);
    }

    printf("Introduce the password(max 32 caracteres): ");
    if (fgets(password, sizeof(password), stdin) == NULL) {
        printf("Error \n");
        exit(1);
    }
    password[strcspn(password, "\n")] = 0; // delete line jump

    memset(key, 0, AES_256_KEY_SIZE);
    size_t keylen = strlen(password);
    if (keylen > AES_256_KEY_SIZE)
        keylen = AES_256_KEY_SIZE;
    memcpy(key, password, keylen);
}

int main() {
    uint8_t input[AES_BLOCK_SIZE];
    uint8_t key[AES_256_KEY_SIZE];
    uint8_t output[AES_BLOCK_SIZE];

    get_user_input(input, key);

    printf("\nencrypted message (hex):\n");
    print_hex(input, AES_BLOCK_SIZE);

    printf("\npassword (hex):\n");
    print_hex(key, AES_256_KEY_SIZE);

    AES_256_decrypt(input, key, output);

    printf("\nDescrypted message (hex):\n");
    print_hex(output, AES_BLOCK_SIZE);

    printf("\nDecrypted message (text):\n");
    for (int i = 0; i < AES_BLOCK_SIZE && output[i]; ++i) {
        putchar(output[i]);
    }
    printf("\n");

    return 0;
}
